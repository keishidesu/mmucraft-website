  ...
  methods: {
    onSubmit (evt) {
      ...
      this.$axios
        .$post(process.env.api, {
          action: 'create',
          target: 'pending-password-reset',
          email: this.form.email,
          token: this.form.token
        })
        .then((res) => {
          if (res.status === 0) {
            this.makeToast('Password reset requested. Email is on the way, might need to take up to 10 minutes to arrive!')
          } else if (res.status === 1) {
            this.makeToast('This email address has not been registered.', true)
          } else if (res.status === 2) {
            this.makeToast('Recaptcha failed, please try again.', true)
          } else if (res.status === 3) {
            this.makeToast('For some reason we cannot send you the reset email, please contact admin for help.', true)
          } else {
